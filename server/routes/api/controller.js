...
exports.addKeyboard = (req, res) => {
  ...
  if (duplicateData.length) return res.status(200).json({ message: "이미 등록 된 정보에요." });
  if (duplicateEmail.length >= 5) return res.status(200).json({ message: "메일 당 5건까지 등록돼요." });
  if (keyboardTable.length >= 100) return res.status(200).json({ message: "대기열이 모두 찼네요." });
...
exports.removeKeyboard = (req, res) => {
  ...
  if (!removeData.length) return res.status(200).json({ message: "조회 된 정보가 없네요." });
