...
exports.addKeyboard = (req, res) => {
  ...
  if (duplicateData.length) return res.status(200).json({ resultCode: "400", message: "이미 등록된 정보에요." });
...
exports.removeKeyboard = (req, res) => {
  ...
  if (!removeData.length) return res.status(200).json({ resultCode: "400", message: "조회된 정보가 없네요." });
  ...
  return res.status(200).json({ resultCode: "100", message: "삭제되었어요." });
};

// [GET] 메일로 등록한 내용 조회
exports.getKeyboard = (req, res) => {
  const { email } = req.query;
  if (!email) return res.status(400).json({ resultCode: "400", message: "메일을 정확히 입력하세요." });
